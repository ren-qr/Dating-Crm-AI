import type { AiConfig } from "@/agent-system/shared/ai/config";
...
export async function completeAiText(
  ...
  if (config.provider === "deepseek") {
    ...
    if (!response.ok) {
      throw new AiProviderError(payload?.error?.message ?? `DeepSeek 调用失败：${response.status}`);
    }

    const answer = payload?.choices?.[0]?.message?.content;
    if (!answer) {
      throw new AiProviderError("DeepSeek 未返回有效回答");
    }
  ...
  if (config.provider === "ollama") {
    ...
    if (!response.ok) {
      ...
      throw new AiProviderError(detail.includes("model") && detail.includes("not found")
        ? `本地模型未找到：${config.model}，请在 Ollama 中确认模型名称后重新保存。`
        : detail);
    }

    const answer = payload?.message?.content;
    if (!answer) {
      throw new AiProviderError("Ollama 未返回有效回答");
    }
